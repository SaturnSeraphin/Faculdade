...
def fixa_primeiro(s):
    if len(s) < 2:
        return s
    primeiro = s[0]
    return primeiro + s[1:].replace(primeiro, '*')
...
def mistura2(a, b):
    a_primeiro = a[:2] if len(a) >= 2 else a
    b_primeiro = b[:2] if len(b) >= 2 else b
    
    a_resto = a[2:] if len(a) >= 2 else ''
    b_resto = b[2:] if len(b) >= 2 else ''
    
    return b_primeiro + a_resto + ' ' + a_primeiro + b_resto
